...
# # Face
# FACE_REFERENCE_PATH = "profiles/face_reference.npy"
# # ---------------------------------------
...
#         self.setWindowTitle("Multi-Biometric Person Identification")
#         self.resize(900, 650)
...
#         self.mode_btn = QPushButton("Switch to FACE Recognition")
#         self.train_btn = QPushButton("Add GAIT Training Videos")
#         self.build_btn = QPushButton("Build GAIT Reference")
#         self.live_btn = QPushButton("Start Live Camera")
#         self.video_btn = QPushButton("Verify Recorded Video")
#         self.stop_btn = QPushButton("Stop")
...
#         # ---------------- MODE STATE ----------------
#         self.current_mode = RecognitionMode.GAIT
...
#         # ================= GAIT STATE =================
#         self.gait_extractor = SkeletonExtractor()
#         self.gait_matcher = None
#         self.gait_embeddings = []
#         self.gait_reference_ready = False
...
#         self.face_reference_embeddings = None

#         # ---------------- SIGNALS ----------------
#         self.train_btn.clicked.connect(self.add_gait_training_videos)
#         self.build_btn.clicked.connect(self.build_gait_reference)
#         self.live_btn.clicked.connect(self.start_live)
#         self.video_btn.clicked.connect(self.verify_video)
#         self.stop_btn.clicked.connect(self.stop_video)
...
#         if self.current_mode == RecognitionMode.GAIT:
#             self.current_mode = RecognitionMode.FACE
#             self.mode_btn.setText("Switch to GAIT Recognition")
#             QMessageBox.information(self, "Mode", "FACE recognition enabled")
#             self.init_face_engine()
#         else:
#             self.current_mode = RecognitionMode.GAIT
#             self.mode_btn.setText("Switch to FACE Recognition")
#             QMessageBox.information(self, "Mode", "GAIT recognition enabled")

#     # =================================================
#     # FACE INITIALIZATION
#     # =================================================
#     def init_face_engine(self):
#         if self.face_finder is not None:
#             return

#         if not os.path.exists(FACE_REFERENCE_PATH):
#             QMessageBox.warning(
#                 self, "Face Error",
#                 "Face reference embeddings not found."
#             )
#             return

#         self.face_reference_embeddings = np.load(FACE_REFERENCE_PATH, allow_pickle=True)
#         self.face_finder = VideoFinder(
#             reference_embeddings=self.face_reference_embeddings
#         )
...
#     # =================================================
#     def process_frame(self, frame):
#         if self.current_mode == RecognitionMode.FACE:
#             self.process_face_frame(frame)
#         else:
#             self.process_gait_frame(frame)

#     # =================================================
#     # FACE PIPELINE (REUSES YOUR webcam.py)
#     # =================================================
...
#         label = "FACE FOUND" if found else "FACE NOT FOUND"
...
#     # =================================================
#     # GAIT PIPELINE (UNCHANGED CORE LOGIC)
#     # =================================================
#     def add_gait_training_videos(self):
#         files, _ = QFileDialog.getOpenFileNames(
#             self, "Select GAIT Training Videos", "", "Video Files (*.mp4 *.avi *.mov)"
#         )
#         if len(files) < 3:
...
#             seq = []
#             while cap.isOpened() and len(seq) < 100:
#                 ret, frame = cap.read()
#                 if not ret:
#                     break
#                 joints = self.gait_extractor.extract_from_frame(frame)
#                 if joints is not None:
#                     seq.append(joints)
#             cap.release()
...
#             if self.gait_matcher is None:
#                 self.gait_matcher = GaitMatcher(GAIT_MODEL_PATH, num_joints=12)

#             emb = self.gait_matcher.embed(seq)
#             self.gait_embeddings.append(emb)

#         QMessageBox.information(self, "GAIT", "Training videos added")

#     def build_gait_reference(self):
#         if len(self.gait_embeddings) < 3:
#             QMessageBox.warning(self, "Error", "Add more GAIT videos")
#             return

#         ref = self.gait_matcher.build_reference(self.gait_embeddings)
#         np.save(GAIT_REFERENCE_PATH, ref)
#         self.gait_reference_ready = True
#         QMessageBox.information(self, "GAIT", "GAIT reference built")

#     def process_gait_frame(self, frame):
#         joints = self.gait_extractor.extract_from_frame(frame)
#         if joints is not None:
#             self.skeleton_buffer.append(joints)
#             if len(self.skeleton_buffer) > self.GAIT_WINDOW:
#                 self.skeleton_buffer.pop(0)

#             if self.gait_reference_ready and len(self.skeleton_buffer) >= self.GAIT_WINDOW:
#                 seq = np.array(self.skeleton_buffer, dtype=np.float32)
#                 ref = np.load(GAIT_REFERENCE_PATH)
#                 emb = self.gait_matcher.embed(seq)
#                 found, _ = self.gait_matcher.match(emb, ref)
...
from gui.gait_window import GaitWindow
